...
/obj/vehicle/multitile
	...
	var/mob_size_required_to_hit = MOB_SMALL
...
/obj/vehicle/multitile/Initialize()
	...
	if(bound_width > world.icon_size || bound_height > world.icon_size)
		...
		lighting_holder.set_light_on(vehicle_light_range || vehicle_light_power) todomatt: lights*/
	/*else if(light_range)
		set_light_on(TRUE)*/
...
/obj/vehicle/multitile/proc/toggle_cameras_status(on)
	if(camera)
		camera.set_status(on)
	if(camera_int)
		camera_int.set_status(on)
...
/obj/vehicle/multitile/healthcheck()
	...
	/*if(health <= 0 && lighting_holder.light_range)
		lighting_holder.set_light_on(FALSE) todomatt: lights*/
	update_icon()
